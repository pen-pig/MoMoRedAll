/*
 * MoMoRedAll v2.4 — PLT Hook Native Library
 * ==========================================
 * 移植自原 MomoRedAll-Magisk v2.3，去除 Zygisk 依赖，
 * 改为通过 JNI 加载的纯 native 库。
 *
...
static void plthook_init() {
    is_target = detect_target_process();
    if (!is_target) return;

    LOG_D("MoMoRedAll v2.4 PLT init — target process detected");
...
// ============================================================
// JNI 入口 — 由 Kotlin 侧 System.loadLibrary("plthook") 自动调用
// ============================================================
extern "C" JNIEXPORT jint JNICALL JNI_OnLoad(JavaVM* vm, void* reserved) {
    LOG_D("JNI_OnLoad — MoMoRedAll PLT Hook library loaded");
    plthook_init();
    return JNI_VERSION_1_6;
}

// ============================================================
// 导出 PLT 替换函数 — 覆盖 libc 同名符号
// ============================================================
extern "C" {
